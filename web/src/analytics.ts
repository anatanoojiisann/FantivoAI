...
export const appSessionId = crypto.randomUUID();
...
export function initAnalytics(enabled: boolean, acquisition: AcquisitionProperties) {
  ...
  try {
    ...
    posthog.register({ app_channel: "telegram_mini_app", app_session_id: appSessionId, funnel_version: 2, ...acquisition });
    initialized = true;
    return true;
  } catch {
    return false;
...
export function identifyUser(telegramId: number, properties: Properties, firstTouchProperties: Properties) {
  ...
  try { posthog.identify(`telegram_${telegramId}`, properties, firstTouchProperties); } catch { /* Analytics must not block creation. */ }
}

export function track(event: string, properties: Properties = {}, options?: CaptureOptions) {
  if (!initialized) return;
  try { posthog.capture(event, properties, options); } catch { /* Analytics must not block creation. */ }
}

export async function flushAnalytics() {
  if (!initialized) return;
  try { await posthog.shutdown(); } catch { /* Navigation must remain available. */ }
}
